...
void OnTriggerEnter2D(Collider2D col) {
  ...
  void OnTriggerExit2D(Collider2D col){
    ...
    if(col.gameObject.name =="Tunel"){
        AudioManager.Instance.IniciarEfectoDefault(); // si pongo tunel en edit in play now, deselecciono esta funcion y salgo con el personaje de la zona del tunel, detecta que he cambiado y suena Default, pero no detecta al entrar al tunel 
    }
     if(col.gameObject.name =="Burbuja"){
        AudioManager.Instance.IniciarEfectoDefault();  //el gFx no consigo activarlo en la burbuja 
